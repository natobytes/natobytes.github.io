...
- **SEO infra (site root):** `robots.txt` (allows all, points to the sitemap), `sitemap.xml` (currently lists **only** the homepage `https://natobytes.com/` — `farmaperto/` and the privacy pages were intentionally left out and can be added later), and `.nojekyll` (stops GitHub Pages running Jekyll on this hand-written site). All canonical URLs use the apex `https://natobytes.com` (per `CNAME`).
